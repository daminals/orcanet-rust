...
//}
//
//async fn create_test_market() -> Market {
//    let peer = std::env::var("BOOTSTRAP_PEER").unwrap();
//    let bootstrap_peers = vec![peer.parse().unwrap()];
//    Market::new(&bootstrap_peers, None, None).await.unwrap()
//}
//
//#[tokio::test]
...
//        price: 100,
//    };
//    let market = create_test_market().await;
//    market.register_file(user.id, user.name, user.ip, user.port, user.price, user.file_hash, chunk_metadata)
